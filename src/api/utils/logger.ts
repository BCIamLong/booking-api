...
const { transportConfig } = loggerConfig

// * if we custom the transport object and then pass it into the pino logger() then we can pass it as the second argument
const log = logger(
  {
    base: {
      pid: 'longhoang'
    },
    timestamp: () => `,"time":"${dayjs().format('YYYY-MM-DDT:HH:mm:ss')}"`
  },
  transportConfig
)
